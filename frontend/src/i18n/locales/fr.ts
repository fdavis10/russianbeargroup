import type { TranslationContent } from "../types";

/** French — amounts in EUR (approx. of EN USD figures) */
export const fr: TranslationContent = {
  site: {
    title: 'IRC "RUSSIAN BEAR"',
    tagline: "Représentant officiel du Ministère de la Défense de la Fédération de Russie",
    email: "support@irc-russianbear.army",
    whatsapp:
      "https://wa.me/79167787177?text=Bonjour%2C%20je%20suis%20int%C3%A9ress%C3%A9",
    whatsapp_phone: "+7 916 778-71-77",
    telegram_admin: "https://telegram.me/ircrussianbear",
    telegram_channel: "https://telegram.me/icr_russianbear",
    facebook: "https://www.facebook.com/people/IRC-Russian-Bear/61591894922054/",
    vk: "https://vk.com/svo_recruit",
    rights: "Tous droits réservés.",
  },
  nav: {
    conditions: "Conditions",
    advantages: "Avantages",
    contact: "Contacts",
    about: "Notre message",
    reviews: "Avis",
    services: "Services",
    faq: "FAQ",
    media: "Médias",
    menu: "Menu",
    language: "Langue",
    openMenu: "Ouvrir le menu",
    closeMenu: "Fermer le menu",
  },
  hero: {
    tagline: 'Agence internationale de recrutement « Russian Bear »',
    taglineSecondary:
      "Représentant officiel du Ministère de la Défense de la Fédération de Russie",
    headline:
      "Recrutement de citoyens d'Afrique, d'Orient, d'Asie et d'Amérique latine pour le service militaire sous contrat — accompagnement complet de la candidature à l'arrivée.",
    subheadline:
      "Aide aux documents, accompagnement visa, hébergement, repas et transfert — soutien complet en Russie. Du début des démarches à l'arrivée en Fédération de Russie : environ 7 à 14 jours.",
    cta: "Envoyer une candidature",
    consultation: "Obtenir une consultation",
    warning: {
      headline:
        "Ne traitez pas avec des intermédiaires — la plupart sont des escrocs qui vous mettent en danger",
      details:
        "Contactez-nous uniquement via les canaux officiels indiqués sur ce site.",
    },
  },
  contactsSection: {
    title: "Contacts",
    links: {
      telegramAdmin: "Telegram",
      whatsappAdmin: "WhatsApp",
      telegramChannel: "Chaîne Telegram",
      facebook: "Facebook",
    },
  },
  conditionsSection: {
    title: "Conditions pour les candidats",
    items: [
      { label: "Âge", value: "20–55 ans" },
      {
        label: "Santé",
        value:
          "absence de maladies graves (VIH, hépatites B/C), absence de troubles psychiques graves, résistance au stress",
      },
      {
        label: "Condition physique",
        value: "pouvoir faire 10 pompes et 20 squats",
      },
    ],
  },
  advantages: [
    "Salaire à partir de 210 000 roubles (3 000 $ — selon le taux de change au moment de la signature)",
    "Équipement tactique moderne fourni intégralement",
    "Prime à la signature de 1 050 000 roubles (15 000 $ — selon le taux de change au moment de la signature)",
    "Obtention de la citoyenneté russe (sur demande)",
    "Aide aux transferts d'argent internationaux (vers la famille)",
    "Aide à l'emploi après la fin du contrat (sur demande)",
    "Accompagnement juridique complet à chaque étape",
    "Participation aux programmes sociaux d'État pour les vétérans",
    "Aide aux soins médicaux gratuits et aux indemnités d'assurance en cas de blessure (8 000–32 000 $)",
    "Aide aux indemnités d'assurance en cas de décès (60 000 $ — aux proches via l'ambassade de la Fédération de Russie)",
    "Cours spécialisé de formation au combat",
  ],
  services: [
    "Traitement des documents clé en main (visas, cartes de migration, assurances, invitations)",
    "Achat de billets (avion, train, mer)",
    "Accompagnement juridique et social complet pendant toute la durée du service en Fédération de Russie",
  ],
  countries: [
    { name: "Russie", code: "RU" },
    { name: "Biélorussie", code: "BY" },
    { name: "Kazakhstan", code: "KZ" },
    { name: "Ouzbékistan", code: "UZ" },
    { name: "Inde", code: "IN" },
    { name: "Serbie", code: "RS" },
    { name: "Cuba", code: "CU" },
    { name: "Arménie", code: "AM" },
  ],
  countryOptions: [
    { value: "Russia", label: "Russie", code: "RU" },
    { value: "Belarus", label: "Biélorussie", code: "BY" },
    { value: "Kazakhstan", label: "Kazakhstan", code: "KZ" },
    { value: "Uzbekistan", label: "Ouzbékistan", code: "UZ" },
    { value: "India", label: "Inde", code: "IN" },
    { value: "Serbia", label: "Serbie", code: "RS" },
    { value: "Cuba", label: "Cuba", code: "CU" },
    { value: "Armenia", label: "Arménie", code: "AM" },
    { value: "Vietnam", label: "Viêt Nam", code: "VN" },
    { value: "Kyrgyzstan", label: "Kirghizistan", code: "KG" },
    { value: "Other", label: "Autre pays", code: "OTHER" },
  ],
  processSection: {
    title: "Processus d'inscription",
    steps: [
      "Candidature",
      "Consultation",
      "Traitement des documents pour l'entrée en Fédération de Russie",
      "Transfert",
      "Signature du contrat",
      "Cours de formation au combat",
      "Début du service",
    ],
  },
  reviews: {
    title: "Avis",
    items: [
      {
        name: "Rajesh K.",
        country: "Inde",
        text: "J'ai obtenu la citoyenneté après un an de service. La société a tout géré — des documents aux billets. Honnêtement !",
        avatar:
          "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=200&h=200&fit=crop&crop=face",
      },
      {
        name: "Marko P.",
        country: "Serbie",
        text: "Je le recommande à tous ceux qui veulent s'engager légalement. Tous les documents ont été traités rapidement.",
        avatar:
          "https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?w=200&h=200&fit=crop&crop=face",
      },
      {
        name: "Andrei V.",
        country: "Biélorussie",
        text: "Équipe professionnelle. Ils m'ont aidé pour le visa, les billets et m'ont accueilli à l'aéroport.",
        avatar:
          "https://images.unsplash.com/photo-1519085360753-af0119f7cbe7?w=200&h=200&fit=crop&crop=face",
      },
      {
        name: "Carlos M.",
        country: "Cuba",
        text: "Tout est transparent et par contrat. Les juristes ont expliqué chaque étape. Merci pour le soutien !",
        avatar:
          "https://images.unsplash.com/photo-1531384441138-2736e62e0919?w=200&h=200&fit=crop&crop=face",
      },
    ],
  },
  servicesSection: {
    title: "Comment nous aidons",
    quote: "« Pas de promesses verbales — des obligations fixées par contrats »",
  },
  faq: {
    title: "FAQ",
    items: [
      {
        question:
          "Que se passe-t-il si je suis blessé, et qui reçoit les paiements si je meurs ?",
        answer:
          "La société signe avec vous un accord d'accompagnement juridique et social pour toute la durée du service. Nous garantissons que les paiements ne vont qu'à la personne de confiance que vous désignez dans vos documents. En cas de blessure, de retards de transferts ou de perte de contact, nos juristes traitent directement avec les hôpitaux et le commandement. En cas de décès, nous aidons les proches à traiter et recevoir rapidement toutes les indemnités d'État via les ambassades (la société n'a pas accès aux fonds eux-mêmes).",
      },
      {
        question:
          "Pourquoi est-il dangereux d'essayer de gagner du temps en voyageant avec un visa touristique et en envoyant vos documents à différents intermédiaires ?",
        answer:
          "Entrer en Russie avec un visa touristique sans soutien direct se termine dans 90 % des cas par une expulsion à l'aéroport et une interdiction d'entrée de 20 ans. Les intermédiaires peu scrupuleux ne s'intéressent qu'à une commission rapide pour les papiers — qu'on vous laisse passer la frontière ou non ne les concerne pas. Nous travaillons uniquement officiellement : visas à but précis, contrôle frontalier réussi et accueil à l'aéroport.\n\nImportant : si vous acceptez de travailler avec notre représentant mais envoyez aussi vos documents à d'autres agents, vos données sont dupliquées dans les bases officielles. À cause de cette duplication, vous êtes automatiquement retiré des listes d'invitation. Dans ce cas, le risque d'expulsion à l'arrivée est de 100 %.",
      },
      {
        question:
          "Est-il vrai que les agents versent 36 000–45 000 € dès la signature du contrat ?",
        answer:
          "Non. Les conditions de prime sont individuelles pour chaque pays. Les promesses des intermédiaires sont une tromperie dans 99 % des cas, pour attirer les gens et collecter des commissions. Les intermédiaires non officiels citent souvent votre futur salaire pour une année entière de service ou cachent d'énormes commissions, de sorte que vous ne recevez qu'une petite partie. Nous assurons une transparence totale : le montant réel du paiement unique et toutes les dépenses possibles sont fixés dans le contrat que vous signez avant de partir pour la Russie.",
      },
      {
        question: "Qui m'aidera à obtenir la citoyenneté russe ?",
        answer:
          "Les intermédiaires ordinaires ne promettent un passeport qu'oralement, tandis que le Ministère de la Défense a d'autres priorités et ne gère pas les questions migratoires. Notre société prend en charge le cycle complet de l'accompagnement juridique. Nous préparons, déposons et suivons tous les documents afin qu'environ 6 mois après la signature du contrat vous deveniez officiellement citoyen de la Fédération de Russie.",
      },
      {
        question:
          "On dit que je serai envoyé directement dans les tranchées sans formation. Est-ce vrai ?",
        answer:
          "Non, c'est exclu. La société dispose d'un centre d'entraînement dont la tâche principale est de vous enseigner les compétences de combat nécessaires pour survivre. Avant le début du service, vous devez suivre un cours de préparation.",
      },
      {
        question:
          "Si j'ai une spécialisation étroite (par exemple médecin ou ingénieur), puis-je servir dans ma profession ?",
        answer:
          "Oui, c'est possible, mais le simple fait d'avoir un diplôme à l'entrée dans le pays ne suffit pas. Tout dépend de votre volonté de fournir des efforts : il faudra apprendre le russe (y compris la terminologie professionnelle), confirmer vos qualifications et suivre des cours de reconversion. Pendant le service, vous aurez la possibilité de suivre une formation militaire spécialisée, et après l'obtention de la citoyenneté russe vous pourrez officiellement travailler dans votre domaine dans le secteur civil.",
      },
    ],
  },
  media: {
    title: "Médias à notre sujet",
    subtitle: "Couverture médiatique mondiale d'IRC « Russian Bear »",
    enlarge: "Cliquez pour agrandir",
    close: "Fermer",
    items: [
      {
        src: "/media/cnn_1.jpg",
        outlet: "BBC",
        alt: "BBC : ICR Russian Bear autorisé comme recruteur officiel de citoyens étrangers",
      },
      {
        src: "/media/cnn_2.jpg",
        outlet: "RT",
        alt: "RT : Trois sociétés de recrutement russes ont signé un mémorandum avec les Forces armées",
      },
      {
        src: "/media/cnn_3.jpg",
        outlet: "New York Post",
        alt: "New York Post : Bear, Shield et Redut — statut officiel du recrutement militaire",
      },
    ],
  },
  form: {
    title: "Envoyer une candidature",
    subtitle: "Remplissez le formulaire — nous vous contacterons sous peu",
    name: "Nom *",
    phone: "Téléphone *",
    country: "Pays *",
    message: "Message",
    submit: "Envoyer la candidature",
    submitting: "Envoi...",
    namePlaceholder: "Votre nom",
    phonePlaceholder: "+33 6 00 00 00 00",
    messagePlaceholder: "Votre question (facultatif)",
    countryPlaceholder: "Sélectionnez un pays",
    dialCodePlaceholder: "Indicatif",
    phoneNationalPlaceholder: "6 00 00 00 00",
    dialSearchPlaceholder: "Rechercher un pays ou un indicatif",
    errors: {
      nameRequired: "Veuillez indiquer votre nom",
      nameMin: "Minimum 2 caractères",
      phoneRequired: "Veuillez indiquer votre téléphone",
      phoneInvalid: "Numéro de téléphone invalide",
      dialRequired: "Choisissez l'indicatif du pays",
      countryRequired: "Veuillez sélectionner un pays",
      submitFailed: "Échec de l'envoi. Veuillez réessayer plus tard.",
    },
    success:
      "Candidature envoyée avec succès. Nous vous contacterons sous peu.",
  },
  consultationForm: {
    headline: "Réserver une consultation",
    description:
      "Les représentants régionaux officiels d'IRC « Russian Bear » examineront attentivement votre situation et proposeront la feuille de route la plus adaptée pour que vous puissiez commencer le service le plus tôt possible.",
    benefits: [
      "Nous expliquons les règles, conditions et le processus d'engagement actuels",
      "Nous clarifions les détails juridiques et la procédure de traitement des documents",
      "Un agent dédié reste en contact avec vous 24 h/24 et 7 j/7",
    ],
    name: "Nom *",
    phone: "Téléphone *",
    phoneHint: "WhatsApp ou Telegram doit être installé sur le numéro indiqué",
    question: "Votre question *",
    submit: "Envoyer la question",
    submitting: "Envoi...",
    namePlaceholder: "Votre nom",
    phonePlaceholder: "+33 6 00 00 00 00",
    phoneNationalPlaceholder: "6 00 00 00 00",
    dialCodePlaceholder: "Indicatif",
    dialSearchPlaceholder: "Rechercher un pays ou un indicatif",
    questionPlaceholder: "Décrivez ce que vous souhaitez savoir",
    errors: {
      nameRequired: "Veuillez indiquer votre nom",
      nameMin: "Minimum 2 caractères",
      phoneRequired: "Veuillez indiquer votre téléphone",
      phoneInvalid: "Numéro de téléphone invalide",
      dialRequired: "Choisissez l'indicatif du pays",
      questionRequired: "Veuillez saisir votre question",
      questionMin: "Minimum 10 caractères",
      submitFailed: "Échec de l'envoi. Veuillez réessayer plus tard.",
    },
    successTitle: "Merci !",
    successMessage: "Nous vous recontacterons !",
    sendAnother: "Poser une autre question",
  },
  sections: {
    advantages: "Avantages",
  },
  errors: {
    loadFailed:
      "Échec du chargement des données. Démarrez le backend : python manage.py runserver",
    retry: "Réessayer",
  },
  pages: {
    contacts: {
      title: "Contacts",
      intro:
        "Contactez-nous uniquement via les canaux officiels. Tous les moyens de joindre IRC « Russian Bear » sont listés ici.",
      directTitle: "Contact direct",
      directHint: "E-mail et téléphone pour une réponse rapide",
      emailLabel: "E-mail",
      phoneLabel: "Téléphone / WhatsApp",
      phoneOnlyLabel: "Téléphone",
      channelsTitle: "Messageries et réseaux",
      channelsHint: "Comptes officiels et chaîne",
      vkLabel: "VKontakte",
      representativesTitle: "Directeurs régionaux",
      representativesEmailHint:
        "Vous avez des questions, des objections ou une situation particulière dans votre région ? Écrivez à cette adresse — le directeur régional vous répondra officiellement et concrètement.",
      representatives: [
        {
          region: "Afrique",
          name: "Kamanga",
          role: "Responsable de la direction régionale Afrique",
          photo: "/africa_agent.jpg",
          email: "support-africa@irc-russianbear.army",
          phone: "+23415550142",
          telegram: "AFR_russianbear",
        },
        {
          region: "Asie",
          name: "Wei",
          role: "Responsable de la direction régionale Asie",
          photo: "/asian_agent.jpg",
          email: "support-asia@irc-russianbear.army",
          phone: "+862155501873",
          telegram: "ASIA_russianbear",
        },
        {
          region: "Orient",
          name: "Zafar",
          role: "Responsable de la direction régionale Orient",
          photo: "/west_agent.jpg",
          email: "support-me@irc-russianbear.army",
          phone: "+97145550261",
          telegram: "ME_russianbear",
        },
        {
          region: "Amérique latine",
          name: "Carlos",
          role: "Responsable de la direction régionale Amérique latine",
          photo: "/latina_agent.jpg",
          email: "support-latam@irc-russianbear.army",
          phone: "+551155550398",
          telegram: "LATAM_russianbear",
        },
      ],
      ctaText: "Besoin d'une consultation ? Laissez une question sur la page d'accueil — nous répondrons.",
      ctaButton: "Réserver une consultation",
    },
    about: {
      title: "Notre message",
      intro:
        "Votre chemin direct vers la Russie. Sans tromperie, sans intermédiaires, avec honneur et respect.",
      sections: [
        {
          heading: "Nous savons pourquoi vous faites ce choix",
          body: "Nous savons ce qui pousse les gens à quitter leur foyer. Ce n'est pas une vie facile qui amène un homme à prendre les armes et à partir à l'autre bout du monde. Derrière chaque décision de ce genre se cache une histoire personnelle forte, le désir de subvenir aux besoins de sa famille et l'espoir d'un avenir nouveau et digne.\n\nVous êtes prêts à risquer ce que vous avez de plus précieux : votre vie. Et nous traitons ce choix avec le plus profond respect. C'est un grand honneur pour nous d'être aux côtés de tels hommes.",
        },
        {
          heading: "L'amère vérité sur les intermédiaires",
          body: "Pendant longtemps, le chemin vers la Russie a été barré par un mur d'agents cupides, d'intermédiaires et d'escrocs. Nous avons vu des gens donner leurs derniers sous pour des promesses vides, attendre des visas pendant des mois, vivre dans la misère dans des pays étrangers, puis se voir refuser l'entrée à la frontière. Le centre international de recrutement « Russian Bear » a perdu des dizaines de milliers de dollars en essayant de travailler avec ces « assistants » malhonnêtes.\n\nAlors nous avons dit : ça suffit.",
        },
        {
          heading: "Nous travaillons en direct",
          body: "Nous avons renoncé par principe et pour toujours à tout intermédiaire. Nous sommes allés directement vers les gens. Aujourd'hui, « Russian Bear » n'est pas une simple agence. Nous sommes un représentant officiel agissant dans l'intérêt du ministère de la Défense de la Fédération de Russie. Nous disposons de nos propres managers de confiance dans les pays d'Amérique latine, d'Afrique, d'Asie et du Moyen-Orient.\n\nNous ne vendons pas de visas. Nous offrons la fraternité, un partenariat honnête et un accompagnement complet. Nous vous prenons par la main dans votre pays natal et ne vous lâchons pas tant que vous n'avez pas construit votre nouvelle vie en Fédération de Russie.",
        },
        {
          heading: "Avec nous, vous ne serez jamais livré à vous-même. Nous garantissons :",
          body: "• Une logistique parfaite et un passage de frontière garanti à 100 %. Toutes les questions de formalités sont réglées rapidement, sûrement et officiellement. Vous aurez toujours un toit et de quoi manger à chaque étape du parcours. Vous ne serez pas seul.\n\n• Une véritable préparation au combat. Nous ne jetons personne dans l'inconnu. Vous serez formé par les meilleurs instructeurs pour être prêt à tout.\n\n• Des arrières solides pour votre famille. Nous prenons en charge le transfert de l'argent que vous gagnez à vos proches. Honnêtement, rapidement et jusqu'au dernier centime.\n\n• Une liaison permanente. Si vous êtes en mission et ne pouvez pas répondre, nous restons en contact avec votre famille. Vos proches ne resteront jamais dans l'ignorance.\n\n• Une protection juridique et sociale complète 24h/24 et 7j/7. Nous sommes à vos côtés en toute situation. Nous assurons les meilleurs soins médicaux en cas de blessure et veillons strictement à ce que toutes les indemnités d'assurance — en cas de blessure ou de décès — parviennent à vos proches via l'ambassade sans retard.",
        },
        {
          heading: "Plus qu'un contrat",
          body: "Le contrat militaire est une étape importante, mais ce n'est que la première. Notre objectif principal est de vous aider à trouver un nouveau foyer. Nous accompagnons le processus d'obtention de la citoyenneté russe dès un mois après le début du service, et après la fin du contrat, nous aidons pour les études, le logement et l'obtention d'un bon emploi dans la vie civile.\n\nVous venez défendre la Russie, et la Russie, à travers nous, vous prend sous sa protection, vous et votre famille.",
        },
      ],
      placeholderNote:
        "Choisissez le chemin direct, honnête et sûr. Bienvenue dans la famille « Russian Bear » !",
    },
  },
  pageTitle:
    'IRC "RUSSIAN BEAR" — Recrutement international pour le service sous contrat',
};
